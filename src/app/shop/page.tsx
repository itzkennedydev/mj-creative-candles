    ...
    <main className="min-h-screen bg-white pb-[80px] lg:pb-0">
          ...
          <div className="py-[32px] lg:py-[40px]">
            <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-[24px]">
              <div className="flex flex-col">
                <h1 className="text-[32px] lg:text-[40px] leading-[130%] font-bold text-black/[0.72]">
                  Shop
                </h1>
                <span className="text-[16px] lg:text-[20px] leading-[130%] text-black/[0.44] mt-[4px]">
                  Custom embroidery and apparel
                </span>
              </div>

              {/* Shop Selector Tabs - NEO Style */}
              <div className="relative inline-flex p-[4px] bg-black/[0.03] rounded-md border border-black/[0.06]">
                <button
            ...
            <div className="mt-[24px] flex gap-[16px]">
              <div className="flex-1 relative">
                <Search className="absolute left-[16px] top-1/2 transform -translate-y-1/2 h-[18px] w-[18px] text-black/[0.44]" />
                <input
                  type="text"
                  placeholder="Search products..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full pl-[48px] pr-[24px] py-[14px] bg-black/[0.03] rounded-md text-[14px] leading-[130%] placeholder:text-black/[0.44] focus:outline-none focus:bg-black/[0.06] transition-all duration-[0.25s]"
                />
              </div>
      ...
      <Container>
        <div className="py-[40px]">
          <div className="lg:grid lg:grid-cols-[280px_1fr] lg:gap-[40px]">
            {/* Sidebar - Desktop */}
            ...
            {showFilters && (
              <div className="lg:hidden fixed inset-0 z-[200] bg-black/50" onClick={() => setShowFilters(false)}>
                <div 
                  className="absolute right-0 top-0 h-full w-[80%] max-w-[320px] bg-white shadow-2xl"
                  onClick={(e) => e.stopPropagation()}
                >
                  <div className="p-[24px] border-b border-black/[0.06]">
                    <div className="flex items-center justify-between">
                      <h2 className="text-[18px] leading-[130%] font-bold text-black/[0.72]">Filters</h2>
                      <button
                        onClick={() => setShowFilters(false)}
                        className="p-[8px] hover:bg-black/[0.03] rounded-full transition-colors"
                      >
                  ...
                  <div className="p-[24px] overflow-y-auto h-[calc(100%-80px)]">
                    <ShopSidebarNeo shopType={activeShop} />
                  </div>
                </div>
              </div>
...
function ShopSidebarNeo({ shopType }: { shopType: string }) {
  ...
  return (
    <div className="space-y-[32px]">
      {/* Categories */}
      <div>
        <h3 className="text-[14px] leading-[130%] font-bold text-black/[0.72] mb-[16px]">Categories</h3>
        <div className="space-y-[8px]">
          {categories.map((category) => (
            <Button
              ...
              className={`w-full justify-start px-[16px] py-[12px] rounded-[12px] text-[14px] leading-[130%] ${
      ...
      {/* Price Range */}
      <div>
        <h3 className="text-[14px] leading-[130%] font-bold text-black/[0.72] mb-[16px]">Price Range</h3>
        <div className="space-y-[8px]">
          {['Under $25', '$25 - $50', '$50 - $100', 'Over $100'].map((range) => (
            <label key={range} className="flex items-center gap-[12px] cursor-pointer group">
              <input
                type="checkbox"
                className="w-[18px] h-[18px] rounded-[4px] border-[2px] border-black/[0.12] checked:bg-[#74CADC] checked:border-[#74CADC]"
              />
              <span className="text-[14px] leading-[130%] text-black/[0.72] group-hover:text-black transition-colors">
                {range}
              </span>
            </label>
          ))}
      ...
      {/* Size Filter */}
      <div>
        <h3 className="text-[14px] leading-[130%] font-bold text-black/[0.72] mb-[16px]">Size</h3>
        <div className="grid grid-cols-3 gap-[8px]">
          {['XS', 'S', 'M', 'L', 'XL', 'XXL'].map((size) => (
            <Button
              key={size}
              variant="outline"
              className="px-[12px] py-[8px] bg-black/[0.03] hover:bg-black/[0.06] text-black/[0.72] rounded-[8px] text-[14px] leading-[130%] transition-all duration-[0.25s] hover:border-black/[0.12] border-[2px]"
            >
              {size}
            </Button>
          ))}
