...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get order ID from session metadata OR find by stripe session ID
    ...
    if (orderId) {
      // Legacy flow: Order ID in metadata
      order = await ordersCollection.findOne({ _id: new ObjectId(orderId) });
    } else {
      // New flow: Find order by stripe session ID (created by webhook)
      order = await ordersCollection.findOne({ stripeSessionId: sessionId });
    }

    if (!order) {
      // Order might not be created yet by webhook - return 202 Accepted
      console.log(
        `Order not found for session ${sessionId} - webhook may still be processing`,
      );
      return NextResponse.json(
        {
          success: true,
          message: "Payment successful - order is being processed",
          note: "Webhook is creating the order and will send confirmation emails",
        },
    ...
    if (order.emailsSent) {
      console.log(
        `Emails already sent for order ${order.orderNumber} - likely sent by webhook`,
      );
      return NextResponse.json({
        success: true,
    ...
    try {
      ...
      if (updateResult.matchedCount === 0) {
        console.log(
          `Emails already being sent or sent for order ${order.orderNumber} - skipping duplicate`,
        );
        return NextResponse.json({
          success: true,
          message: "Emails already sent",
