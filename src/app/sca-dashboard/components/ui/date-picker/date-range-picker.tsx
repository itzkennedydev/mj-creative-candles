...
const DateRangePickerInner = ({
  ...
  // Global keyboard shortcuts - work even when popover is closed
  useEffect(() => {
    if (!presets || presets.length === 0 || disabled) return;
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (!e.metaKey && !e.ctrlKey && !e.altKey && e.key.length === 1 && e.key.match(/[a-z0-9]/i)) {
        const pressedKey = e.key.toLowerCase();
        const preset = presets.find(
          (p) => p.shortcut?.toLowerCase() === pressedKey && !p.requiresUpgrade
        );
  ...
  }, [presets, disabled, onPresetSelected]);

  // Keyboard shortcuts are also handled directly in the Presets component via Command when open
