    ...
    <PopoverPrimitive.Root open={openPopover} onOpenChange={setOpenPopover}>
      ...
      <PopoverPrimitive.Portal>
        <PopoverPrimitive.Content
          ...
          side={side}
          className={cn(
            "animate-slide-up-fade border-border-subtle bg-white z-50 items-center rounded-lg border drop-shadow-lg sm:block",
            popoverContentClassName,
          )}
          sticky={sticky}
          ...
          onWheel={onWheel}
          onPointerDownOutside={(e) => {
            // Allow clicks on other filter chips, filter buttons, or filter remove buttons to work
            const target = e.target as Element;
            if (
              target.closest('[data-filter-chip]') ||
              target.closest('[data-filter-button]') ||
              target.closest('[data-filter-remove]') ||
              target.closest('[aria-haspopup="dialog"]')
            ) {
              // Don't prevent the default behavior, allow the click to proceed
              return;
            }
          }}
        >
