...
import { fetcher } from "@sca/utils";
import Link from "next/link";
import { useEffect, useState } from "react";
import useSWR from "swr";

export function SCAStatusBadge({ className }: { className?: string }) {
  const { data } = useSWR<{
    ongoing_incidents: {
      name: string;
  ...
  }>("https://status.sca.co/api/v1/summary", fetcher);
  ...
  useEffect(() => {
    ...
    if (ongoing_incidents.length > 0) {
      ...
      if (incident) {
        const { current_worst_impact, name } = incident;
        const color =
