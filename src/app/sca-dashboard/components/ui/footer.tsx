...
import { cn, createHref } from "@sca/utils";
...
import { SCAStatusBadge } from "./sca-status-badge";
import {
  DubProduct,
  DubProductIcon,
  Github,
...
const socials = [
  ...
  },
  {
    name: "GitHub",
    icon: Github,
    href: "https://github.com/sca/dashboard",
  },
  {
    name: "YouTube",
    icon: YouTube,
...
const navigation = {
  product: [
    ...FEATURES_LIST.filter(({ title }) => title !== "SCA Integrations").map(
      ({ id, title, href }) => ({
        id,
        name: title,
        href,
    ...
    { id: null, name: "SCA Enterprise", href: "/enterprise" },
  ],
  solutions: [
  ...
  resources: [
    ...
    {
      name: "Affiliates",
      href: "https://partners.sca.co/sca",
      target: "_blank",
    },
  ],
  company: [
...
export function Footer({
  ...
  let { domain = "sca.co" } = useParams() as { domain: string };
  if (staticDomain) {
    domain = staticDomain;
  }
    ...
    >
      <footer>
        ...
        <div className="mt-12 grid grid-cols-1 items-center gap-8 sm:grid-cols-3">
          ...
          >
            <Image
              src="https://assets.sca.co/misc/soc2.svg"
          ...
          <p className="text-xs text-neutral-500 sm:text-right">
            © {new Date().getFullYear()} SCA Technologies, Inc.
          </p>
        </div>
      </footer>
    </MaxWidthWrapper>
