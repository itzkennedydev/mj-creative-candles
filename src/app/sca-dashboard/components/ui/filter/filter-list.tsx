...
export function FilterList({
  ...
  // Flatten all filter chips (handles arrays properly)
  const filterChips = activeFilters.flatMap(({ key, value: filterValue }) => {
              if (key === "loader") {
      return [
                  <motion.div
              ...
              if (!filter) {
      // Still render the filter chip even if filter definition not found
      // This prevents the FilterList from not showing when filters are selected
      // Handle both single values and arrays
      const valuesToRender = Array.isArray(filterValue) ? filterValue : [filterValue];
      
      return valuesToRender.map((value, idx) => (
        <motion.div
          key={`${key}-${value}-${idx}`}
          initial={{ opacity: 0, y: 4 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-center gap-2.5 px-3 py-2">
            <span className="shrink-0 text-neutral-600">
              <svg height="18" width="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg" className="h-4 w-4">
                <g fill="currentColor">
                  <path d="M3.75,3.25h7.5c.552,0,1,.448,1,1v5H3.75" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
                  <path d="M12.25,5.75h2c.552,0,1,.448,1,1v4c0,.552-.448,1-1,1h-3.5c-.552,0-1-.448-1-1v-1.5" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
                  <line fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" x1="10.043" x2="12.25" y1="11.457" y2="9.25"></line>
                  <line fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" x1="3.75" x2="3.75" y1="1.75" y2="16.25"></line>
                </g>
              </svg>
            </span>
            {key}
          ...
          <div className="flex items-center">
            <div className="sm:inline-flex flex items-center gap-2.5 px-3 py-2">
              {String(value)}
            </div>
          </div>
          <button
    ...
    const valuesToRender = filter.multiple && Array.isArray(filterValue)
                  ? filterValue
      : [filterValue];
    
    return valuesToRender.map((value) => {
                ...
                const OptionDisplay = ({
                  ...
                  <div
                    className={cn(
                      "sm:inline-flex flex items-center gap-2.5 px-3 py-2",
                      className,
                    )}
                  >
                    <span className="shrink-0 text-neutral-600">
                  ...
                  >
                    ...
                    <div className="flex items-center">
                      ...
                      onSelect && !filter.hideInFilterDropdown ? (
                        ...
                        <div className="sm:inline-flex flex items-center gap-2.5 px-3 py-2 cursor-pointer hover:bg-neutral-50">
                          {OptionDisplay({})}
                        </div>
                      )}
                    </div>
    ...
    >
      <div className={cn("flex w-full flex-wrap items-center gap-x-4 gap-y-2", className)}>
        ...
        {activeFilters?.length !== 0 && (
          <button
            type="button"
            className="group flex items-center gap-2 whitespace-nowrap rounded-lg border border-transparent px-3 py-2 text-sm text-neutral-500 ring-inset ring-neutral-500 transition-colors hover:border-neutral-200 hover:bg-white hover:text-black focus:outline-none w-fit"
            onClick={onRemoveAll}
          >
            Clear Filters
