...
import { X, Loader2, Mail, Phone, MessageSquare, MapPin, ChevronLeft, ChevronRight } from "lucide-react";
...
export function OrderDetailModal({ order, open, onOpenChange, onOrderUpdated }: OrderDetailModalProps) {
        ...
        const fetchProductsAndMatch = async () => {
          try {
            ...
            order.items.forEach((item: { productId: string }) => {
              ...
              const product = allProducts.find((p: { id: string; _id?: string }) => 
                p.id === productId || 
              ...
              if (product) {
                ...
                if (product.images && Array.isArray(product.images) && product.images.length > 0) {
                  product.images.forEach((img: { imageId?: string; dataUri?: string; image?: string }) => {
  ...
  const allProductImages = useMemo(() => {
    ...
    order.items.forEach((item) => {
      const productId = item.productId;
      if (productId && productImages[productId] && productImages[productId].length > 0) {
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-0 z-40 m-auto h-fit w-full border border-neutral-200 bg-white p-0 shadow-xl sm:rounded-2xl scrollbar-hide animate-scale-in overflow-y-auto max-w-screen-xl">
          ...
          <div className="flex flex-col items-start gap-2 px-6 py-3 md:flex-row md:items-center md:justify-between border-b border-neutral-200">
            <div className="flex min-w-0 max-w-full items-center gap-1">
              <div className="flex min-w-0 items-center gap-2 px-1.5">
                <span className="min-w-0 truncate text-sm font-semibold text-neutral-800">Order {order.orderNumber}</span>
              </div>
            </div>
          ...
          <div className="grid w-full gap-4 max-md:overflow-auto md:grid-cols-[1.5fr_1fr] max-md:max-h-[calc(100dvh-200px)] max-md:min-h-[min(566px,_calc(100dvh-200px))] md:[&>div]:max-h-[calc(100dvh-200px)] md:[&>div]:min-h-[min(566px,_calc(100dvh-200px))]">
            <div className="scrollbar-hide px-6 md:overflow-auto">
              <div className="flex min-h-full flex-col gap-4 pt-6 pb-0">
                ...
                <div className="bg-neutral-50 rounded-lg p-4">
                  <div className="space-y-3">
                    {order.items.map((item, idx) => {
                        ...
                        const productImage = item.productId ? productImages[item.productId] : null;
            ...
            <div className="scrollbar-hide px-6 md:overflow-auto md:pl-0 md:pr-4">
              <div className="bg-white rounded-lg pt-6">
                <h2 className="text-sm font-medium text-neutral-700 mb-3 pr-6">Product Images</h2>
                {allProductImages.length > 0 ? (
                  <div className="relative w-full aspect-square rounded-lg overflow-hidden bg-neutral-100 border border-neutral-200 mb-4">
                    ...
                    <div className="relative w-full aspect-square rounded-lg overflow-hidden bg-neutral-100 border border-neutral-200 flex items-center justify-center mx-6 mb-4">
              ...
              <div className="relative flex flex-col gap-3">
                ...
                <div className="mb-0 pb-6">
                  <label className="block text-xs font-medium text-neutral-600 mb-1.5">
                    ...
                    className="block w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-sm text-neutral-900 placeholder-neutral-400 focus:border-neutral-500 focus:outline-none focus:ring-1 focus:ring-neutral-500 min-h-[80px] resize-y"
